...
from utils.utils import is_admin, charger_config, sauvegarder_config, log_erreur, is_non_verified_user, role_autorise
        ...
        try:
            ...
            for demande in demandes:
                ...
                if user:
                    try:
                        await salon.send(
                            f"⏰ Rappel : {user.mention} ({demande.get('nom')} {demande.get('prenom')}) attend toujours une validation."
                        )
                    except Exception:
                        pass
        except Exception as e:
