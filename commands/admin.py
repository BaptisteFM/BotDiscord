...
class AdminCommands(commands.Cog):
    ...
    async def autoriser_commande_autocomplete(
        self,
        interaction: discord.Interaction,
        current: str
    ) -> list[app_commands.Choice[str]]:
        """
        Autocomplete pour autoriser_commande : liste commandes + catégories.
        """
        # 1) toutes les commandes slash
        all_cmds = [cmd.name for cmd in self.bot.tree.get_commands()]
        # 2) toutes les clés (catégories) dans permissions.json
        all_cats = list(charger_permissions().keys())
        # 3) fusion et filtre
    ...
    async def retirer_commande_autocomplete(
        self,
        interaction: discord.Interaction,
        current: str
    ) -> list[app_commands.Choice[str]]:
        """
        Autocomplete pour retirer_commande : liste commandes + catégories.
        """
        all_cmds = [cmd.name for cmd in self.bot.tree.get_commands()]
        all_cats = list(charger_permissions().keys())
        choices = all_cmds + all_cats
